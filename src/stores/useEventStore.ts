...
import { supabase } from "@/lib/supabase/clientSupabase"; // Votre client Supabase
...
export const useEventStore = defineStore("event", () => {
  ...
  const stepErrors = computed(() => {
    ...
    if (currentStep.value === 3) {
      if (!d.timezone)
        errors.timezone = "Veuillez sélectionner un fuseau horaire.";
      d.dates.forEach((date, i) => {
        if (!date.starts_at)
          errors[`start-${i}`] =
            `La date de début est requise (créneau ${i + 1}).`;
        if (!date.ends_at)
          errors[`end-${i}`] = `La date de fin est requise (créneau ${i + 1}).`;
        if (date.starts_at && date.ends_at && date.ends_at <= date.starts_at)
          errors[`dates-${i}`] =
            `La fin doit être après le début (créneau ${i + 1}).`;
      });
    }
    ...
    if (currentStep.value === 4 && !d.isFree) {
      ...
      d.ticketCategories.forEach((cat, i) => {
        if (!cat.name.trim())
          errors[`cat-name-${i}`] =
            `Le nom de la catégorie ${i + 1} est requis.`;
        if (cat.price === undefined || cat.price < 0)
          errors[`cat-price-${i}`] =
            `Le prix de la catégorie ${i + 1} est invalide.`;
        if (!cat.quantity || cat.quantity <= 0)
          errors[`cat-qty-${i}`] =
            `La quantité de la catégorie ${i + 1} est invalide.`;
      });
    }
  ...
  function removeTicketCategory(id: number) {
    if (eventData.value.ticketCategories.length > 1)
      eventData.value.ticketCategories =
        eventData.value.ticketCategories.filter((c) => c.id !== id);
  }
   ...
   */
  async function submitEvent(): Promise<{
    success: boolean;
    eventId?: string;
  }> {
    ...
    try {
      ...
      if (d.dates.length > 0) {
        ...
        if (datesError)
          throw new Error(`Erreur créneaux : ${datesError.message}`);
      }
