...
const archivedEvents = computed(
  () => events.value.filter((e) => e.status === "archived").length,
);

const totalTickets = computed(() =>
  events.value.reduce(
    (sum, e) =>
      sum + (e.ticket_types?.reduce((s, t) => s + (t.quantity ?? 0), 0) ?? 0),
    0,
  ),
);
...
const categoryStats = computed(() => {
  ...
  return Object.entries(map)
    ...
    .map(([name, count]) => ({
      name,
      count,
      pct: Math.round((count / totalEvents.value) * 100),
    }));
});
...
const formatCurrency = (n: number) => n.toLocaleString("fr-FR") + " XOF";
  ...
  <div class="min-h-screen font-sans">
    ...
    <div v-if="isLoading" class="flex items-center justify-center min-h-screen">
      <div class="flex flex-col items-center gap-4">
        <div
          class="w-10 h-10 border-4 border-[#EF233C]/20 border-t-[#EF233C] rounded-full animate-spin"
        />
    ...
    <div v-else class="max-w-[1400px] mx-auto space-y-8">
      ...
      >
        <div>
          <div
            class="flex items-center gap-2 text-xs font-semibold text-slate-400 uppercase tracking-widest mb-1"
          >
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <!-- Événements -->
        <div
          class="bg-white rounded border border-slate-100 p-5 shadow-sm hover:shadow-md transition-shadow"
        >
          <div class="flex items-center justify-between mb-4">
            <div
              class="w-10 h-10 bg-[#EF233C]/10 rounded flex items-center justify-center"
            >
          ...
          <div
            class="flex gap-2 mt-3 pt-3 border-t border-slate-50 text-[10px] font-bold text-slate-400"
          >
            <span class="flex items-center gap-1"
              ><span
                class="w-1.5 h-1.5 rounded-full bg-emerald-400 inline-block"
              ></span
              >{{ publishedEvents }} publiés</span
            >
            <span class="flex items-center gap-1"
              ><span
                class="w-1.5 h-1.5 rounded-full bg-amber-400 inline-block"
              ></span
              >{{ draftEvents }} brouillons</span
            >
          </div>
        </div>
        ...
        >
          <div class="flex items-center justify-between mb-4">
            <div
              class="w-10 h-10 bg-violet-50 rounded flex items-center justify-center"
            >
          ...
          <p class="text-3xl font-bold text-slate-900">
            {{ totalTickets.toLocaleString("fr-FR") }}
          </p>
          <p class="text-xs text-slate-400 font-medium mt-1">
            Places disponibles
          </p>
        ...
        <!-- Revenu potentiel -->
        <div
          class="bg-slate-900 rounded p-5 shadow-sm hover:shadow-lg transition-shadow"
        >
          <div class="flex items-center justify-between mb-4">
            <div
          ...
          <p class="text-xs text-white/40 font-medium mt-1">
            Revenus potentiels
          </p>
          <div
            class="mt-3 pt-3 border-t border-white/10 text-[10px] font-bold text-white/30"
          >
        ...
        >
          ...
          >
            <span class="flex items-center gap-1"
              ><Archive :size="11" />{{ archivedEvents }} archivés</span
            >
            <span class="flex items-center gap-1"
              ><Zap :size="11" class="text-[#EF233C]" />{{ draftEvents }} en
              attente</span
            >
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
          <!-- Événements récents -->
          <div
            class="bg-white rounded border border-slate-100 shadow-sm overflow-hidden"
          >
            <div
              class="flex items-center justify-between px-6 py-4 border-b border-slate-50"
            >
              <div>
                ...
                <p class="text-xs text-slate-400 mt-0.5">
                  Vos {{ recentEvents.length }} derniers événements
                </p>
              </div>
              <button
            ...
            >
              <div
                class="w-14 h-14 bg-slate-50 rounded flex items-center justify-center mb-3"
              >
            ...
            <div v-else class="divide-y divide-slate-50">
              ...
              >
                <!-- Thumbnail -->
                <div
                  class="w-12 h-12 rounded overflow-hidden bg-slate-100 shrink-0"
                >
                  <img
                ...
                <div class="flex-1 min-w-0">
                  <p
                    class="text-sm font-bold text-slate-800 truncate group-hover:text-[#EF233C] transition-colors"
                  >
                    {{ event.title }}
                  </p>
                  <div
                    class="flex items-center gap-3 mt-0.5 text-[11px] text-slate-400 font-medium"
                  >
                    <span class="flex items-center gap-1">
                    ...
                    <span class="flex items-center gap-1">
                      <MapPin
                        v-if="event.location_type === 'in-person'"
                        :size="11"
                      />
                      ...
                      {{
                        event.location_type === "online"
                          ? "En ligne"
                          : event.city || event.country || "Lieu non défini"
                      }}
                    </span>
                  </div>
                </div>
                ...
                <div class="flex flex-col items-end gap-1.5 shrink-0">
                  <span
                    :class="[
                      ...
                      statusMeta(event.status).bg,
                      statusMeta(event.status).text,
                    ]"
                  >
                    <span
                      :class="[
                ...
                <ChevronRight
                  :size="15"
                  class="text-slate-300 group-hover:text-[#EF233C] transition-colors shrink-0"
                />
              </div>
            </div>
          </div>
          ...
          <div class="grid grid-cols-3 gap-4">
            ...
            >
              <div
                :class="[
                  'w-9 h-9 rounded flex items-center justify-center mb-3',
                  item.bg,
                ]"
              >
        ...
        <div class="space-y-6">
          ...
          <div class="bg-slate-900 rounded p-5 shadow-sm">
            <h2 class="text-sm font-bold text-white tracking-tight mb-4">
              Actions rapides
            </h2>
            <div class="space-y-2">
              <router-link
          ...
          <div class="bg-white rounded border border-slate-100 p-5 shadow-sm">
            <h2 class="text-sm font-bold text-slate-900 tracking-tight mb-4">
              Répartition par catégorie
            </h2>
              ...
              <div v-for="cat in categoryStats" :key="cat.name">
                <div class="flex items-center justify-between mb-1">
                  <div class="flex items-center gap-2">
                    ...
                    />
                    <span
                      class="text-xs font-semibold text-slate-700 capitalize"
                      >{{ cat.name }}</span
                    >
                  </div>
                  <div class="flex items-center gap-2">
          ...
          <div class="bg-white rounded border border-slate-100 p-5 shadow-sm">
            <h2 class="text-sm font-bold text-slate-900 tracking-tight mb-1">
              Types de tickets
            </h2>
            ...
            <div v-else class="space-y-2">
              <div
                v-for="event in recentEvents.filter(
                  (e) => e.ticket_types?.length,
                )"
                :key="event.id"
                class="rounded bg-slate-50 p-3"
              >
                <p class="text-xs font-bold text-slate-700 truncate mb-2">
                  {{ event.title }}
                </p>
          ...
          <!-- Statut compte -->
          <div
            class="bg-gradient-to-br from-[#EF233C] to-[#D90429] rounded p-5 text-white shadow-lg shadow-[#EF233C]/20"
          >
            <div class="flex items-center gap-3 mb-3">
              <div
                class="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center text-lg font-bold"
              >
              ...
              <div>
                ...
                <p
                  class="text-[11px] text-white/70 mt-0.5 truncate max-w-[150px]"
                >
                  {{ currentUser?.email }}
                </p>
              </div>
            ...
            <div
              class="border-t border-white/20 pt-3 mt-3 grid grid-cols-2 gap-2 text-center"
            >
              <div>
      ...
      <!-- ══ FOOTER ══ -->
      <div
        class="flex items-center justify-between pt-4 border-t border-slate-100 text-xs text-slate-300 font-medium"
      >
