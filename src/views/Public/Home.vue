...
const getLocation = (event: EventType): string => {
  ...
  return (
    [event.city, event.country].filter(Boolean).join(", ") || "Lieu non défini"
  );
};
...
<template>
  ...
  <header class="w-full bg-[#EDF2F4] border-b border-[#2B2D42]/10">
    <!-- Barre supérieure -->
    <div
      class="hidden md:flex items-center justify-between px-6 py-2 border-b border-[#2B2D42]/5 text-[11px] font-bold text-[#8D99AE]"
    >
      <div class="flex items-center gap-6">
        <div
          class="flex items-center gap-1 cursor-pointer hover:text-[#2B2D42] transition-colors"
        >
          <MapPin :size="14" class="text-[#EF233C]" /> Bénin
      ...
      <div class="flex items-center gap-3 border-r border-[#2B2D42]/10 pr-6">
        <Facebook
          :size="14"
          class="hover:text-[#EF233C] cursor-pointer transition-colors"
        />
        <Instagram
          :size="14"
          class="hover:text-[#EF233C] cursor-pointer transition-colors"
      ...
      <div class="flex items-center gap-2 md:gap-4">
        ...
        >
          <router-link to="/dashboard/create-event"
            >Publier un événement</router-link
          >
        </Button>
        ...
        <div ref="menuRef" class="relative">
          ...
          >
            ...
            <div
              class="size-8 bg-[#2B2D42] rounded-full flex items-center justify-center text-white overflow-hidden"
            >
              <img
            ...
            >
              ...
              <template v-if="currentUser">
                <div class="px-4 py-3 border-b border-[#2B2D42]/5">
                  ...
                  <p class="text-[11px] text-[#8D99AE] truncate">
                    {{ currentUser.email }}
                  </p>
                </div>
                <button
    ...
    <div class="flex flex-col items-center pb-8 space-y-8 mt-2">
      <!-- Onglets -->
      <div
        class="inline-flex p-1.5 bg-[#2B2D42]/5 rounded-full border border-[#2B2D42]/5"
      >
        <button
        ...
        >
          Cotisations
          <Badge
            class="bg-[#EF233C] text-white text-[8px] px-1.5 h-3.5 border-none font-bold"
            >NEW</Badge
          >
        </button>
      </div>
      ...
      <div class="w-full max-w-7xl px-6 relative">
        <div
          class="flex items-center gap-4 overflow-x-auto no-scrollbar p-4 justify-around w-full"
        >
          <button
          ...
          >
            <component
              ...
              :class="
                activeCategory === cat.value
                  ? 'text-[#EF233C]'
                  : 'text-[#8D99AE] group-hover:text-[#EF233C]'
              "
            />
        ...
        <button
          class="absolute -right-2 top-1/2 -translate-y-1/2 size-12 bg-white rounded-full flex items-center justify-center border border-[#2B2D42]/10 hover:bg-[#EDF2F4] transition-all shadow-xl z-10"
        >
  ...
  <main class="min-h-screen bg-[#EDF2F4]">
    <div class="max-w-7xl mx-auto px-6 py-12">
      ...
      <div class="flex items-center justify-between mb-10">
        <div>
          <h2 class="text-3xl font-black text-[#2B2D42] tracking-tighter">
            <template v-if="searchQuery">
              Résultats pour "<span class="text-[#EF233C]">{{
                searchQuery
              }}</span
              >"
            </template>
        ...
        <div
          class="hidden md:flex items-center gap-2 text-xs font-black uppercase text-[#2B2D42]/40 tracking-widest"
        >
      ...
      >
        ...
        <p class="text-[#2B2D42] font-bold animate-pulse">
      ...
      >
        ...
        >
          <div class="relative h-52 overflow-hidden bg-[#EDF2F4]">
            ...
            <div
              v-else
              class="w-full h-full flex items-center justify-center bg-gradient-to-br from-[#EDF2F4] to-[#8D99AE]/10"
            >
              <Ticket class="text-[#8D99AE]/30" :size="48" stroke-width="1" />
            </div>

            <div
              class="absolute inset-0 bg-gradient-to-t from-[#2B2D42]/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"
            />

            <div
              class="absolute top-4 left-4 bg-white/80 backdrop-blur-md px-3 py-1 rounded-full text-[9px] font-black text-[#2B2D42] uppercase tracking-[0.15em] shadow-sm border border-white/20"
            >
          ...
          <div class="p-6 flex flex-col flex-1">
            <div
              class="flex items-center gap-2 text-[#EF233C] text-[10px] font-black uppercase mb-3"
            >
              <span class="tracking-wider">{{ formatDate(event) }}</span>
              <span class="size-1 bg-[#8D99AE]/30 rounded-full shrink-0" />
              <span class="truncate tracking-wider opacity-80">{{
                getLocation(event)
              }}</span>
            </div>

            <h3
              class="text-lg font-bold text-[#2B2D42] leading-tight mb-1 group-hover:text-[#EF233C] transition-colors line-clamp-2"
            >
              {{ event.title }}
            </h3>
            ...
            <p class="text-[11px] text-[#8D99AE] font-medium mb-6">
              Par
              <span class="text-[#2B2D42] font-bold">{{
                event.owner_name || "Inconnue"
              }}</span>
            </p>

            <div
              class="mt-auto pt-4 border-t border-[#EDF2F4] flex items-center justify-between"
            >
              <div>
                <p
                  class="text-[9px] text-[#8D99AE] font-black uppercase tracking-[0.1em] mb-0.5"
                >
                  Tarif
                </p>
                <p
                  class="text-base font-black tracking-tight"
                  :class="event.is_free ? 'text-green-600' : 'text-[#2B2D42]'"
                >
              ...
              <div
                class="size-10 rounded-xl bg-[#EDF2F4] text-[#2B2D42] flex items-center justify-center group-hover:bg-[#EF233C] group-hover:text-white group-hover:rotate-90 transition-all duration-500 shadow-sm"
              >
      ...
      <div
        v-else
        class="text-center py-24 bg-white rounded-[32px] border-2 border-dashed border-[#8D99AE]/20"
      >
        <div
          class="inline-flex size-16 items-center justify-center bg-[#EDF2F4] rounded-full mb-4"
        >
